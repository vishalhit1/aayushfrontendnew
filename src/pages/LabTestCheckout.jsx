...
import { Button, Modal, Form, Spinner, Accordion } from "react-bootstrap";
...
import deliveryaddress from "../assets/icons/deliveryaddress.png";
import samplecoll from "../assets/icons/samplecoll.png";
...
const LabTestCheckout = () => {
    ...
    const VALUE_ADDON_PRICE = 100; // Per person

    // ❗ Correct: Total patients count (not per test)
    ...
    const totalToPay = labTotal + valueAddOnTotal - discount;
    ...
    const createPayOnCollectionOrder = async () => {
        try {
            setLoading(true);

            const testsWithPatients = Object.entries(selectedPatients)
                .map(([id, patients]) => {
                    const cartItem = labCart.find((x) => x._id === id);
                    if (!cartItem) return null;

                    return {
                        testId: cartItem.itemType === "test" ? id : undefined,
                        packageId: cartItem.itemType === "package" ? id : undefined,
            ...
            toast.success("Order placed successfully! Pay on collection.");
            removeFromLabCart();
            navigate(`/lab-payment-success?order_id=${data.orderId}`);
        } catch (err) {
            console.error(err);
    ...
    const proceedToPayment = async () => {
        ...
        const testsWithPatients = Object.entries(selectedPatients).map(
            ([id, patients]) => {
                ...
                console.log("cartItem>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>", cartItem.itemType)
                return {
                    // pick testId OR packageId based on type
                    testId: cartItem.itemType === "test" ? id : undefined,
                    packageId: cartItem.itemType === "package" ? id : undefined,
        ...
        // const payload = {
        //     testsWithPatients,
        //     valueAddOnTotal:valueAddOnTotal,
        //     address: selectedAddress,
        //     slot: selectedLabSlot,
        ...
        //     uploadedPrescriptions: prescriptions || null
        // };

        const formData = new FormData();

        // Convert your JSON payload to keys in FormData
        formData.append("testsWithPatients", JSON.stringify(testsWithPatients));
        formData.append("valueAddOnTotal", valueAddOnTotal);
        formData.append("address", JSON.stringify(selectedAddress));
        formData.append("slot", JSON.stringify(selectedLabSlot));
        formData.append("totalAmount", grandTotal);
        formData.append("taxBreakup", JSON.stringify({ cgst, sgst, igst }));
        formData.append("couponCode", appliedCoupon?.code || "");
        formData.append("coupon", appliedCoupon?._id || "");
        formData.append("paymentMethod", "online")
        // Append prescription images properly
        prescriptions.forEach((file) => {
            formData.append("uploadedPrescriptions", file);
        });


        console.log("🧾 Sending to createLabTestOrder API:", formData);

        // return false;

        try {
            // :one: Create lab order
            const { data } = await API.post("/api/payment/create-lab-order", formData);
        ...
        <div className="container my-4">
            <div className="row">
                            ...
                            <div key={test._id} className="total-letst-lshn">
                                <div>
                                    ...
                                    <div className="padd-checkout">
                                        ...
                                        {/* ===== ADDRESS SECTION ===== */}
                                        <div className="address-member-checkout">
                                            <h4> <img src={deliveryaddress} className="new-check-images" alt="" />Delivery Address
                                                <i
                                                    style={{ cursor: 'pointer' }}
                                        ...
                                        {/* ===== SLOT SECTION ===== */}
                                        <div className="slot-member-checkout">
                                            <h4><img src={samplecoll} className="new-check-images" alt="" /> Sample Collection Slot
                                                <i
                                                    style={{ cursor: 'pointer' }}
                                                    ...
                                                    data-bs-toggle="tooltip"
                                                    data-bs-placement="top"
                                                    title="Edit Sample Collection Slot"
                                                >
                                                </i>
                                            </h4>
                                            {selectedLabSlot ? (
                                                <div className="info-details">
                                                    <p style={{ marginBottom: "5px" }}>
                                                        {new Date(selectedLabSlot?.date).toLocaleDateString(
                                                            "en-US",
                                                            {
                                                                weekday: "short",
                                                                day: "numeric",
                ...
                <div className="col-lg-4">
                    ...
                    <div className="bill-summary">
                        ...
                        <div className="payment-method-box mb-3">
            ...
            >
                <Modal.Header closeButton>
                    <Modal.Title><img src={patientadds} className="select-patient-labsessd" alt="" /> Select Patients</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    {loading ? (
