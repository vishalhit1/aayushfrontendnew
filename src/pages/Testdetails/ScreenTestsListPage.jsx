...
const ScreenTestsListPage = () => {
  ...
  const fetchScreenTestData = async () => {
    try {
      setLoading(true);
      const res = await API.get(`/api/labtestsdata/three-tests/${name}`, {
        params: { pincode }
      });
  ...
  const EmptyState = ({ title, subtitle }) => (
    <div className="empty-state-container text-center">
      ...
      <button onClick={fetchScreenTestData} className="refresh-offers12w">
        Refresh
      </button>
    </div>
  );
    ...
    <>
      ...
      <Container className="age-categhory-newass">
        <div className="text-center">
          <h2 className="testimonial-heading">{decodeURIComponent(name)}</h2>
        </div>
        ...
        <Row>
          {tests.length === 0 ? (
            <EmptyState
              ...
              subtitle="No tests available for this selection."
            />
          ) : (
            <>
              {tests.map((test) => (
                  ...
                  <div className="labtest-ibndivi-test-abcds">
                    <div className="labtest-ibndivi-test-abcds-sliders">
                      ...
                      {test.actualPrice && test.actualPrice > test.price ? (
                        <a className="percenatge-offerses">
                          {Math.round(
                      ...
                      <div className="labtest-indiv-test-detail">
                        ...
                        <button
                          ...
                          onClick={() => toggleBook(test, "test")}
                        >
                          {isInCart(test._id) ? (
                            <>
                              <i className="fa fa-minus"></i> Added
                            </>
                          ) : (
                            <>
                              <i className="fa fa-plus"></i> Book
                            </>
                          )}
                        </button>
                      </div>
        ...
        <Row>
          {packages.length === 0 ? (
            <EmptyState
              ...
              subtitle="No packages available for this selection."
            />
          ) : (
            <>
              {packages.map((pkg) => (
...
export default ScreenTestsListPage;
