...
import { Container, Row, Col, Accordion, Modal } from "react-bootstrap";
import { toast } from "react-toastify";
import API from "../../api/axios";
import { CartContext } from "../../context/CartContext";
import testincluded from "../../assets/icons/testincluded.png";
import descr from "../../assets/descr.svg";
...
const HolisticPackageDetails = () => {
    ...
    const [counters, setCounters] = useState({ tests: 0, orders: 0, clients: 0 });
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data } = await API.get(`/api/labtestpackage/holistic/${id}`);
    ...
    const startCounterAnimation = () => {
        const duration = 2000;
        const steps = 60;
        const increment = { tests: 1100 / steps, orders: 11 / steps, clients: 1100 / steps };
        let currentStep = 0;
        const interval = setInterval(() => {
            currentStep++;
            setCounters({
                tests: Math.min(Math.floor(increment.tests * currentStep), 1100),
                orders: Math.min(Math.floor(increment.orders * currentStep), 11),
                clients: Math.min(Math.floor(increment.clients * currentStep), 1100),
            });
    ...
    if (!pkg) return <div className="fullpage-loader">
        <div className="spinner"></div>
    </div>;

    const toggleAccordion = (id) => setActiveIndex(activeIndex === id ? null : id);
    ...
    const otherTiers = pkgList.filter((p) => p.tier !== pkg.tier).slice(0, 2);
                                                        ...
                                                        <>
                                                            ...
                                                            {t.includedTests.length > 2 && (
                                                                ...
                                                                >
                                                                    {isExpanded ? (
                                                                        <>
                                                                            View Less <span style={{ fontWeight: "bold", marginLeft: "4px" }}>-</span>
                                                                        </>
                                                                    ) : (
                                                                        <>
                                                                            View More <span style={{ fontWeight: "bold", marginLeft: "4px" }}>+</span>
                                                                        </>
                                                                    )}
                                                                </button>
                                                            )}
...
export default HolisticPackageDetails;